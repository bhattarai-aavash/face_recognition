...
class Student():
    def __init__(self,root,txtuser,txtpass):
        self.root = root
        ...
        Table_frame.place(x=10,y=30,width=680,height=300) 
        ...
        self.fetch_data(txt,pas)


        btn_frame = Frame(Left_frame,bd=2,relief=RIDGE,bg="white")
        btn_frame.place(x=15,y=400,width=710,height=50)
        ...
        right_frame1 = LabelFrame(main_fame,bd=2,bg="white",relief=RIDGE,text="Attendance Details",font=("times new roman",12,"bold"))
        right_frame1.place(x=780,y=370,width=710,height=680) 

        table_frame1 = Frame(right_frame1,bd=2,relief=RIDGE,bg="white")
        table_frame1.place(x=5,y=10,width=700,height=300)

        #scroll bar
        scroll_x = ttk.Scrollbar(table_frame1,orient=HORIZONTAL)
        scroll_y = ttk.Scrollbar(table_frame1,orient=VERTICAL)

        self.AttendanceReportTable = ttk.Treeview(table_frame1,column= ("id","roll","name","department","time","date","attendance"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        scroll_x.pack(side = BOTTOM,fill=X)
        scroll_y.pack(side = RIGHT,fill=Y)
    ...
    def fetch_data(self,txtuser,txtpass):
        conn = mysql.connector.connect(host="localhost",username="root",password="Bipinlm@10",database="project")
        my_cursor = conn.cursor()
        my_cursor.execute("select * from student where Email=%s and DOB=%s",(txtuser,txtpass))
...
def student_login():
        return student_login_window(Toplevel())
