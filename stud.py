...
class Student:
    def __init__(self,root):
        ...
        title_lbl = Label(text="STUDENT REGISTRATION SYSTEM",font=("times new roman",35,"bold"),bg="white",fg="black")
        ...
        #Reset Button
        reset_btn = Button(btn_frame,text="Reset",command=self.reset_data,width=70,font=("times new roman",13,"bold"),bg="blue",fg="white")
        reset_btn.grid(row=3,column=0,padx=5,pady=5)

        #Take_photo Button
        Take_photo_btn = Button(btn_frame,text="Take Photo Sample",command=self.generate_dataset,width=70,font=("times new roman",13,"bold"),bg="blue",fg="white")
        Take_photo_btn.grid(row=5,column=0,padx=5,pady=5)

        #Update_photo Button
        Update_photo_btn = Button(btn_frame,text="Update Photo Sample",width=70,font=("times new roman",13,"bold"),bg="blue",fg="white")
    ...
    def add_data(self):
        if self.var_dep.get()=="Select department" or self.var_std_id.get()=="" or self.var_std_name.get()=="":
            ...
            try:
                conn = mysql.connector.connect(host="localhost",username="root",password="Bipinlm@10",database="facetable")
                my_cursor = conn.cursor()
                my_cursor.execute("insert into details values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",(
                                                                                                            self.var_dep.get(),
                                                                                                            self.var_year.get(),
        ...
        if self.var_dep.get()=="Select department" or self.var_std_id.get()=="" or self.var_std_name.get()=="":
            ...
            try:
                ...
                if Update==True:
                    conn = mysql.connector.connect(host="localhost",username="root",password="Bipinlm@10",database="facetable")
                    my_cursor = conn.cursor()
                    my_cursor.execute("UPDATE details set dep=%s,year=%s,semester=%s,name=%s,section=%s,roll=%s,gender=%s,dob=%s,phone=%s,email=%s,address=%s,photosample=%s where studentId=%s",(
    ...
    def generate_dataset(self):
        if self.var_dep.get()=="Select department" or self.var_std_id.get()=="" or self.var_std_name.get()=="":
            ...
            try:
                ...
                for x in my_result:
                    id+=1
                my_cursor.execute("UPDATE student set Department=%s,Year=%s,Semester=%s,Name=%s,Section=%s,[Roll Number]=%s,Gender=%s,DOB=%s,[Phone Number]=%s,Email=%s,Address=%s,PhotoSample=%s where StudentId=%s",(
